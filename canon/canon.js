...
function Simulation(popSize){
  ...
  this.draw = function(){
    ...
    if (this.genReady){
      ctx.fillText("Generation ready !", 20, 140);
    }
  };
  ...
  this.stepEndCallback = function(id, fitness, ctx){
    ...
    console.log("max fit " + ctx.maxFit);
    console.log(ctx.maxFit);
    ctx.maxFit = Math.max(ctx.maxFit, fitness);
    ctx.genMaxFit = Math.max(ctx.genMaxFit, fitness);
    console.log("math.max " + Math.max(ctx.maxFit, fitness));
